...
public class IndexQueryBenchmark extends Benchmark {

	private static final long RUNTIME_THRESHOLD = 3*60; //seconds
	private static final Long MEMORY_THRESHOLD = 200*1024*1024L;
	private IndexInterface sqt;
	private int empty;
	private int counter;

	public IndexQueryBenchmark(BenchmarkConfig config, String version,
			QTreeHashing hashing) {
		super(config,version ,hashing);
		sqt = IndexFactory.loadIndex(config.getIndexFile(version,_hasher));
	}

	public IndexQueryBenchmark(SemQTree sqt, File query, File inputData,
			File outRoot) {
			...
			while((query=parseQuery(br))!=null){
				...
				while(t!=null && ! t.finished()){
					try {
						...
						if( time || mem){
							log("Q["+counter+"] INTERRUPTED cause:  time:"+time+" sec  mem:"+mem +" ("+((mm.getFreeBytes()/(double)1024)/(double)1024)+" MB)");
							t.interrupt();
							t = null;
							Thread.sleep(1000);
							break;
				...
				log("Q["+counter+"] stopped result:"+(t!=null));
				
				//cleanup
				QueryResultEstimation qre =null;
				if(t != null){
					qre=t.getQueryResultEstimation();
					log("get query result: "+(qre!=null));
				}
				if(qre!=null){
					serialiseQueryResultEstimation(qre, new File(getConfig().queryRoot(),"query_"+sqt.getVersionType()+"-"+_hasher+".qre."+counter+".ser"));
				...
				else{log("Q["+counter+"] no results");empty++;}
				t = null;
